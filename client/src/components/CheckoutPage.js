...
const CheckoutPage = () => {
  ...
  const handleDeliverySubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        // Postcode is valid and within the UK
        if (
          deliveryDetails == null ||
          deliveryDetails.addresses.length === 0 ||
          !deliveryDetails?.address_id
        ) {
      ...
      if (error.response && error.response.status === 404) {
        ...
        console.log(postcode)
        showAlert("An error occurred while validating the postcode.", 'error');
      }
    }
  };
  ...
  const handleAddressSelection = (e, newpostcode, index) => {
  ...
  const handleMakeRequest = async () => {
    ...
    try {
      const message = `Hello, I want to make an order to postcode ${postcode}`
      const token = localStorage.getItem('token');
      const response = await axios.post(
        '/api/users/custom-order-request', // Adjust the endpoint as needed
        {
      ...
      setRequestSent(true); // Mark as sent
      showAlert('Your request has been sent to the seller.', 'success');
    } catch (error) {
  ...
  const renderDeliveryStep = () => (
    ...
    { (currentStep == 1 ? (
        ...
        {deliveryDetails?.addresses &&
          deliveryDetails.addresses.map((address, index) => (
            <div key={address.address_id} className="mb-2">
              <label className="block">
                <input
                  ...
                  onChange={(e) =>
                    handleAddressSelection(e, address.postcode, index)
                  }
                />
        ...
        {isOutsideUk ? (
          <div className="bg-purple-50 p-3 rounded-lg max-w-[400px]">
            <p className="mb-2">
              This address is not UK registered. Please enter a different address or make a request below to contact the seller.
            </p>
            <button
              onClick={handleMakeRequest}
              disabled={isRequesting || requestSent}
              className={`mr-2 px-6 py-2 rounded transition duration-300 ${
      ...
      <div className="flex gap-5">
        <div>{deliveryDetails?.addresses[deliveryAddressIndex].first_line}, {deliveryDetails?.addresses[deliveryAddressIndex].city}, {deliveryDetails?.addresses[deliveryAddressIndex].postcode}, {deliveryDetails?.addresses[deliveryAddressIndex].country}</div>
        <button 
    ...
    <div>
      <div className="mb-4">
        <label className="flex items-center gap-1">
          ...
          width="30px" height="30px" viewBox="0 0 24 24" enable-background="new 0 0 24 24">
        <path fill="#FAFAFA" d="M20,20H4c-1.1,0-2-0.9-2-2V6c0-1.1,0.9-2,2-2h16c1.1,0,2,0.9,2,2v12C22,19.1,21.1,20,20,20z"/>
        <path id="Bottom-Bar" fill="#303F9F" d="M2,16h20v2c0,1.1-0.9,2-2,2H4c-1.1,0-2-0.9-2-2V16z"/>
        <path id="Top-Bar" fill="#FFAB00" d="M4,4h16c1.1,0,2,0.9,2,2v2H2V6C2,4.9,2.9,4,4,4z"/>
        ...
        <path fill="#303F9F" d="M15,13H9c-0.6,0-1-0.4-1-1v0c0-0.6,0.4-1,1-1h6c0.6,0,1,0.4,1,1v0C16,12.6,15.6,13,15,13z"/>
        </svg>
        </label>
    ...
    >
      ...
      <div className="h-[1px] w-full bg-gray-300 mb-1"/>
         <div className="justify-between font-normal w-full flex my-0">
            ...
            <span className="text-[15px]">£{(itemsAmount)?.toFixed(2)}</span>
          ...
          <div className="justify-between font-normal w-full flex mb-1">
            ...
            <span className="text-[15px]">£{(deliveryAmount)?.toFixed(2)}</span>
